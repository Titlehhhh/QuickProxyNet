...
охват/стоимость — gRPC. REALITY — половина корпуса, но это отдельный проект, а не
фича: пока нет способа подделать uTLS-отпечаток, честный `NotSupportedException`
остаётся единственным правильным поведением.
